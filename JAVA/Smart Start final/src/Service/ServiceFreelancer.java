...
public class ServiceFreelancer {
    ...
    public void ajouterFree(Freelancer F) throws SQLException {
        String req = "INSERT INTO `freelancer` ( `ID`,`Nom`,`Prenom`,`MotDePass`,`Email`,`Nationalite`,`CV`,`Description`,`ImageFreelancer`) VALUES (?,?,?,?,?,?,?,?,?)";
    ...
    return sujet;
      ...
      public Freelancer recherche(String Email) throws SQLException
    {
    ResultSet res=ste.executeQuery("select * from freelancer  WHERE Freelancer.Email ='"+Email+"' ;");
    Freelancer sujet=null;
    while (res.next()) {
      sujet=new Freelancer(res.getInt(1), res.getString(2),res.getString(3),res.getString(4),res.getString(5),res.getString(6),res.getString(7),res.getString(8), res.getBytes(9));
       }
